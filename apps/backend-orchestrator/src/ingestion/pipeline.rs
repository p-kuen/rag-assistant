...
impl IngestionPipeline {
    ...
    pub async fn process_document(
        ...
        self.update_task_status(&task_id, TaskStatusType::Processing, None, None).await;
        ...
        tracing::info!("Document '{}' chunked into {} pieces", document.title, chunks.len());

        // Step 2: Generate embeddings for each chunk
        let mut chunks_with_embeddings = Vec::new();
        for (i, mut chunk) in chunks.into_iter().enumerate() {
            let embedding = self.embedding_service.generate_embedding(&chunk.content)
                .await
                .map_err(|e| {
                    self.update_task_status(&task_id, TaskStatusType::Failed, None, Some(e.to_string()));
                    e
            ...
            self.update_task_status(&task_id, TaskStatusType::Processing, progress, None).await;
        }

        tracing::info!("Generated embeddings for {} chunks", chunks_with_embeddings.len());

        // Step 3: Index documents in Meilisearch
        self.meilisearch_service.index_documents(chunks_with_embeddings)
            .await
            .map_err(|e| {
                self.update_task_status(&task_id, TaskStatusType::Failed, None, Some(e.to_string()));
                e
        ...
        self.update_task_status(&task_id, TaskStatusType::Succeeded, Some(1.0), None).await;
    ...
    pub async fn process_text(
        ...
        let chunks = self.chunker.chunk_text(&content, &title)
            .map_err(|e| {
                self.update_task_status(&task_id, TaskStatusType::Failed, None, Some(e.to_string()));
                e
            })?;
